...
服务只使用文件型 SQLite schema v4 的统一 state store，其中有 `operations`、`upload_sessions` 和 `purge_jobs` 三类状态。SQLite 是上传状态的唯一权威，服务不会在共享根内写入、读取或导入 JSON 上传状态文件。CLI `--state-dir` 或 YAML `state-dir` 必须提供一个目录，数据库固定使用 `<state-dir>/state.sqlite3`；没有进程内数据库或单独文件路径配置入口。
