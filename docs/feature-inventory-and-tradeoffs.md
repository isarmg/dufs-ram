...
本文以当前工作树（Cargo 版本 `0.49.5`）的实际代码为准，盘点 Dufs 当前所有对外可见、可配置，以及会显著影响安全性、正确性、性能和可维护性的功能。普通辅助函数和测试夹具不单独作为“功能”列出；最终发布身份必须以制品内 `dufs --version` 的完整 Git SHA 为准。
...
| P-08 | 可验证交付 | fork 托管在 `isarmg/dufs-ram`。版本 tag 触发的 GitHub Release 流程先等待同 tag/SHA 的分层 CI、拒绝 yanked 的依赖审计及真实正式包 E2E；只读 job 构建带完整源码 SHA 的 GNU/Linux x86-64 便捷二进制并从 tagged changelog 提取说明，唯一写权限 job 不 checkout/执行输入，按 artifact ID、聚合摘要、远端状态/size、回下载 SHA-256 与逐字节比较后发布。匹配草稿可安全续跑，异摘要失败关闭。仓库同时提供 MIT/Apache-2.0 双许可证、`SECURITY.md`、部署基线、运维手册，以及生成实际构建环境清单、CycloneDX SBOM、`THIRD_PARTY_LICENSES.txt`、经摘要审核的 Rust 标准库 notice、校验和与 OpenSSL 签名的本地正式发布脚本 | 不影响运行；删除这些材料会失去持续回归、来源追踪、许可声明和制品验证基线；自动便捷二进制没有独立发布者签名，需要正式信任链时仍须使用本地签名包和独立渠道固定的公钥 | 开发/运维 | 中 |
...
| T-18 | 正式发布包真实 E2E | `.github/workflows/formal-release-e2e.yml` 在 `v*` tag、每周和人工触发时，以只读权限在含 shell 元字符的隔离 clone 中建立精确本地版本 tag、生成临时 Ed25519 key，并不带跳过开关调用真实 `package-release.sh`。它经过完整检查、vendor/build/SBOM/checksum/sign/sync/no-clobber 链，随后独立核验外层四项制品、签名/公钥、包内 `SHA256SUMS` 与二进制完整版本/SHA；不引用生产或自定义 secrets，只使用只读 GitHub token checkout，也不上传输出。便捷 Release 必须等待同 tag/SHA 的该任务成功 | 删除后 helper 自测仍可通过，但 exact-tag、隔离总门、真实构建和最终签名发布组合路径可能长期无人执行；临时测试密钥只证明流程，不构成生产信任根 | 开发/运维 |
