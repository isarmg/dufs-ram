...
| U-13 | 精确正文边界 | 超出声明剩余量返回 `413`；正文不足返回 `409`。确认态的剩余量固定为零，已知非空正文会保留 `awaiting-confirmation` 与原绑定值并要求查询，不会伪装成可继续写入的 `running` | 防止请求走私式额外数据和错误成功；确认态降格会诱导客户端向只读满 stage 重放正文 | 保障 | 高 |
