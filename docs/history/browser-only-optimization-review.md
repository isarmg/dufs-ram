...
- 预检只改善交互，不承担原子性。缺少 `X-Dufs-Upload-Overwrite` 或显式 `false` 的 PUT/PATCH 使用原子 no-replace；`true` 必须携带 64 位小写十六进制 target revision。revision 绑定账号摘要、规范根内目标路径和完整 replacement identity，并在 rename 前再次核对。Missing 发布使用 `RENAME_NOREPLACE`，晚到 occupant 不会被覆盖，成功后还会核对目的名称与已打开 stage；Existing 覆盖是 identity 复核后执行普通 rename，不是对共享根外部 writer 的目录项 CAS；
- 保留同一页面、同一 `File` 对象的已知失败重试：先用 HEAD 核对绑定当前账号摘要的服务端记录。SQLite schema v4 的 `upload_sessions` 在内部严格区分 `Running/CommitStarted/AwaitingConfirmation/Committed/Rejected/Unknown`，对外映射为 `running/awaiting-confirmation/committed/rejected/unknown`，响应另接受没有持久记录的 `not-seen` 和仅描述本次请求未启动的 `not-started`。`not-started` 可以显示 Retry，但点击后仍先查询原 ID；只有 offset 未满的 `running` 才能续传 PATCH，满 offset `awaiting-confirmation` 只能明确发布或丢弃，`rejected/not-seen` 换新 ID，`committed` 精确匹配才成功，`CommitStarted/Unknown` 仍按结果未知处理并禁止盲目重试；
- 服务端要求 PUT/PATCH 声明总长度和精确 offset，最多写入本次声明的剩余字节，并继续检查正文确实结束；额外正文返回 `413`；
- 每次 PUT/PATCH 在等待路径租约前建立绝对总 deadline，覆盖租约等待、上传准备、正文读取、写入、flush、metadata 重放和等待最终提交确认。合法头先解析，随后依次取得路径租约、全局上传槽，并以持有二者的受跟踪任务完成 route metadata；fresh PUT 再在任何 stage/祖先/本次状态 mutation 前，以有界分页扫描检查仍有效的 upload/purge 路径义务，路径冲突返回稳定 `409`，状态查询暂不可用返回可重试 `503`，检查超时返回 `408`，三者都带 `not-started` 且不会改变旧记录。后续受跟踪上传 task 仍可只读查询 owner state、目标/stage identity、metadata 和空间；首次 filesystem/upload-state mutation 与总 deadline 原子竞争。deadline 先关闭边界就 abort task，并以 `408 not-started + retry` 证明本次没有越界；边界前未处理的只读 I/O 为 `408/503 not-started + retry`，task 先越界后的外层超时或未处理错误才是 `unknown + query_upload`。为约束慢文件系统准备工作的资源，槽满会直接 `429 not-started` 而不先查询 owner state；前端点击 Retry 后必须 HEAD，届时才会取得旧 ID 的真实 partial/terminal 状态；
...
- 已跨 mutation boundary 后的上传超时、磁盘水位不足、连接取消和强制停止分别保存有效检查点或删除无效暂存，不提前覆盖旧目标；boundary 前的总 deadline/只读 I/O 失败不修改旧检查点；
...
- 存储提交边界区分 `NotPublished`、条件冲突与 `PublishedDurabilityUnknown`：确定发生在发布前的文件同步或条件复核失败会清理旧 stage/控制记录并尽力写入 `Rejected`；目标 revision 冲突不发布目标，保留满 stage 为 `AwaitingConfirmation`。Missing-target rename 成功后若目的 identity 无法再证明对应已打开 stage，则按结果未知处理。用户接受当前目标时以同一 ID、满 offset、空正文 PATCH 和最新 revision 发布，无需重传；目标再次变化会再次确认，并在每次可信 target-change 响应上重新使列表 snapshot 失效，即使用户刚刚刷新过；跳过则调用 `POST /__dufs__/api/upload/discard`。discard 先原位提交 `AwaitingConfirmation→Rejected` 并设置首次终态 TTL，再按 stage identity 尽力清理；已有 `Rejected` 的重试不写库、不延长 TTL，stage 缺失或已被替换时保留当前 occupant 仍返回严格绑定的 `204 rejected`。若 stage 已带旧目标 metadata 而目标随后消失，服务返回 `upload_metadata_preservation_refused`，前端先 discard，再用新 ID 完整 create-only PUT，避免把旧属性带到新文件。rename 后父目录同步失败，或已持久化发布但 `Committed` 终态写入失败，都报告 `unknown`；
- fresh PUT 在创建祖先、stage 和 `upload_sessions` 记录前从最近存在的父目录完成空间准入，空间不足不会留下目录；准入成功后自动创建的祖先带身份记录，后续会话准备在正文前失败时，自底向上只回收仍为空、身份未变且由本请求创建的目录。拥有共享根写权限的外部进程仍属于明确的信任边界：Existing 覆盖的“最后复核 → 普通 rename”微窗不能隔离恶意同 UID writer。
...
- 维护扫描和 delete trash 回收只跨片保存根内相对路径与进程内 cursor，每片重新打开工作目录并关闭临时 fd，打开 fd 数不随目录深度增长。DELETE 在可见 rename 前先把含目标/trash 相对路径和源 dev/inode/类型的 `Prepared` job 写入 SQLite；rename 与父目录 `fsync` 后把完整 32 字节 trash revision 随 `Ready` 原子提交，worker 再 claim 为 `Claimed`。purge I/O 错误把 job 持久化回 `Ready`，attempt 递增并从 100 ms 指数退避到最长 30 秒，没有固定重试次数上限，其他 ready job 可以越过；重启把遗留 `Claimed` 恢复为立即可重试的 `Ready`。`Prepared` 没有已提交 revision，恢复时不按源路径/inode 猜测 rename 结果：保留原目标，把任何 trash occupant 移入 quarantine 后释放 intent。`Ready/Claimed` 同时由 committed revision 与持续 fd 锚点授权；最终候选先改名到随机 disposal 名并按既有 fd 复核，身份异常或已记账根最终删除的 `ENOTEMPTY/EXIST` 会隔离整棵 trash 根、释放 job，不从 cursor 0 重扫。未记账 orphan 只有在通道满、取消或普通 I/O 失败时保持隐藏并留待后续 maintenance；`InvalidData` 会立即隔离整棵根。递归打开使用 `RESOLVE_NO_XDEV`，嵌套或 bind mount 不会被越过。能通过 inotify 观察并竞争随机工作名的恶意同 UID writer 仍在威胁边界之外；
...
- 删除不再为每个请求生成无限后台任务：schema v4 `purge_jobs` 以全局 4096、每账号 1024 的 durable outbox 保存 `Prepared/Ready/Claimed` 和 committed trash revision，单 purge worker 和可从已记账 trash 根重建的进程内分片状态把清理并发、CPU 时间、fd 数与停机响应都变成显式边界。v2 会在一个事务内依次迁移到 v3/v4，v3 在一个事务内增加 revision，其他版本零修改拒绝。轮转调度、ready-job bypass、无固定次数上限的持久指数退避与 `Claimed` 重启恢复共同避免普通 I/O 故障丢 job；缺少 committed revision、身份不一致或最终删除异常则把对象/整根移入永久 quarantine 并释放 job，不再让歧义记录永久占用配额。
...
- 构建把 Git SHA 写入 `dufs --version`；`scripts/package-release.sh` 只接受干净、由与 Cargo 版本一致的精确 tag 标记的提交。完整 `scripts/check.sh` 在已验证 commit archive 的无 Git 私有副本中以 `env -i`、固定工具链和独立 HOME/Cargo/npm/target/tmp 运行：Cargo vendor 后离线；npm 只从 lockfile HTTPS+SHA-512 验证宿主 cache 后播种私有 cache 并 prefer-offline，缺失包/审计仍可能联网。宿主 RustSec Git 数据库只有在 canonical origin、`HEAD=FETCH_HEAD`、实体 `FETCH_HEAD` 不得比当前时间早超过 7 天或晚超过 300 秒，并通过 Git/物理/内容完整性检查后才可复用；否则先在私有数据库中联网刷新。合格输入以无硬链接私有 clone 封存 revision、fetch epoch、index/config 校验和，拒绝 alternates、symlink/submodule/特殊项、untracked 路径和 tracked 内容/mode 不匹配；先以 `cargo audit --db … --no-fetch --no-yanked` 预审计，再把必填 `DUFS_QUALITY_AUDIT_DB` 传给隔离门。封存时校验 seal 与新鲜度，预审计后只重验 seal；完整门禁后重验 seal 与新鲜度，随后销毁质量树和该 RustSec 数据库。门禁后 snapshot index 复验 tracked 内容/mode 和非忽略新增路径，再 fresh extract 用于签名构建；`BUILD-ENVIRONMENT.txt` 只记录 advisory revision/fetch epoch，不宣称记录内部 index/config 摘要。检查后、签名前及发布前继续重验 exact source；预检、隔离快照和解包树都拒绝 symlink、submodule 与特殊文件，并继续拒绝 replace refs、legacy grafts 和私有 attributes。构建/打包两份独立源码 archive 会校验 commit、树、mode、额外路径并比较 SHA-256；
