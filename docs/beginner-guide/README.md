...
文档以**当前工作区源码（Cargo 版本 0.48.0）**为准。Git 标签中的历史版本可能有不同界面、协议或文件结构。
...
- HTTP 超时不总能证明操作失败。如果服务器已经越过不可取消的提交边界，客户端只能得到“结果未知”；普通写操作随后按原 Operation ID 查询，上传则按原 Upload ID 对账。
