...
日志本身也不是无损审计数据库：文件、stdout 和 stderr 共用容量 4096 的有界队列，拥塞时丢弃最新记录，并聚合输出 `log_queue_overloaded dropped_newest=...`。应监控这条告警；一旦出现，相关时间窗内的访问日志可能不完整，不能仅凭“没找到日志”断言请求未发生。
...
目录 ZIP 等已删除能力就是典型例子：旧 `?zip` 请求返回 `410`，旧 YAML 中相关字段会被严格拒绝，必须在升级前清理调用方和配置。
