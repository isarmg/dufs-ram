...
Authorization、Proxy-Authorization、Cookie 和 CSRF 请求头会在自定义日志变量中脱敏。连接处理错误会记录 TCP peer、错误类别和系统错误码，便于定位网关 `502`、超时和协议问题。停机开始后由嵌入式调用送入的晚到请求也会记录完整请求上下文、`503` 和拒绝态，不会绕过访问日志。
