...
#[rstest]
fn zip_get_and_head_are_private_and_never_stored(server: TestServer) -> Result<(), Error> {
    let url = format!("{}?zip", server.url());

    let get = server.request(reqwest::Method::GET, &url).send()?;
    assert_eq!(get.status(), StatusCode::OK);
    assert_private_no_store(get.headers())?;
