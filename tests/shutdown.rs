...
fn staged_upload_paths(
    ...
    let checkpoint = stage.with_file_name(format!(
        "{}.state",
        stage
            .file_name()
            .and_then(|name| name.to_str())
            .ok_or("invalid upload staging name")?
    ));
    Ok((stage, checkpoint))
}
...
fn wait_for_staged_upload(root: &std::path::Path, expected: u64) -> Result<(), Box<dyn Error>> {
    ...
    while Instant::now() < deadline {
        if staged_upload_paths(root)
            .ok()
            .and_then(|(stage, _)| std::fs::metadata(stage).ok())
