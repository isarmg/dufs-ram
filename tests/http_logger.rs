...
fn verified_session_user_is_written_to_access_log(tmpdir: TempDir) -> Result<(), Error> {
    ...
    let (mut child, port) = spawn_logged_server(
        &tmpdir,
        &[
            "--auth",
            USER_ACCOUNT,
...
fn only_successful_embedded_asset_gets_are_omitted_from_access_log(
    ...
    let (mut child, port) = spawn_logged_server(&tmpdir, &args)?;
    let session = login_with_uri_prefix(port, TEST_USER, TEST_PASSWORD, uri_prefix)?;
    let client = Client::new();
    let page_path = uri_prefix.to_string();
    let page = client
    ...
    let download_path = format!("{uri_prefix}test.txt");
    let response = client
    ...
    let api_path = format!("{uri_prefix}__dufs__/api/mkdir");
    let response = client
    ...
    let unauthorized_path = format!("{index_js_path}?unauthorized=1");
    let response = client
        .get(format!("http://localhost:{port}{unauthorized_path}"))
        .send()?;
    assert_eq!(response.status(), 401);
    assert_eq!(
        response
    ...
    let wrong_method_path = format!("{index_js_path}?wrong-method=1");
    let response = client
        .head(format!("http://localhost:{port}{wrong_method_path}"))
        .header(COOKIE, &session.cookie)
        .send()?;
    assert_eq!(response.status(), 404);
    assert_eq!(
    ...
    let login_path = format!("{uri_prefix}__dufs__/login");
    for expected in [
        ...
        format!("P006 GET {unauthorized_path} 401"),
        format!("P006 GET {missing_path} 404"),
        format!("P006 HEAD {wrong_method_path} 404"),
    ] {
        assert!(
...
fn spawn_logged_server(tmpdir: &TempDir, args: &[&str]) -> Result<(Child, u16), Error> {
    let mut child = Command::new(assert_cmd::cargo::cargo_bin!())
        .arg(tmpdir.path())
        .arg("-p")
        .arg("0")
...
fn login(port: u16, username: &str, password: &str) -> Result<Session, Error> {
    login_with_uri_prefix(port, username, password, "/")
}

fn login_with_uri_prefix(
    port: u16,
    username: &str,
    password: &str,
    uri_prefix: &str,
    ...
    let form = form_urlencoded::Serializer::new(String::new())
        ...
        .finish();
    let response = client
        .post(format!("http://localhost:{port}{uri_prefix}__dufs__/login"))
    ...
    let cookie = response
        ...
        .to_string();
    let page = Client::new()
        .get(format!("http://localhost:{port}{uri_prefix}"))
