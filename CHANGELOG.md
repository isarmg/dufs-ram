...
本文件记录本项目的所有重要变更。“未发布”描述当前工作树；带版本号和日期的条目保留对应版本当时的行为，若与“未发布”冲突，不应把旧条目误读为当前实现。
...
- 删除成功后的首屏目录刷新若与后台隐藏回收清理重叠，会对经过协议校验的 `directory_changed`/`refresh_target` 冲突自动重试一次；连续冲突仍停在显式 Retry 状态，避免无限请求并消除瞬时冲突造成的空列表错误。
