...
> **发布状态：** [Dufs 0.49.7 GitHub Release](https://github.com/isarmg/dufs-ram/releases/tag/v0.49.7) 已于 `2026-08-26T18:23:46Z` 公开并成为 latest，且不是草稿或预发布版本；附注标签 `v0.49.7` 未签名，当前实际解引用到提交 `5b098e2a8f05557b72efdf7929f4ccef3a3af837`。[只读 CI](https://github.com/isarmg/dufs-ram/actions/runs/32997046391)、[依赖审计](https://github.com/isarmg/dufs-ram/actions/runs/32997046157)、[正式签名包 E2E](https://github.com/isarmg/dufs-ram/actions/runs/32997046548) 与 [Linux Release 无写权限验证/构建](https://github.com/isarmg/dufs-ram/actions/runs/32997048979) 四类历史门禁均在首次 attempt 成功；这些运行所验证提交与当前 tag commit 的源码 tree 相同，但 commit identity 不同，不能把运行记录视为当前 tag commit 的签名或精确 SHA 证明。唯一持有 `contents: write` 的发布 job 只消费同一 workflow attempt 的已验证输入，并完成远端元数据、摘要、大小、资产集合与实际回下载复核。Release 正文与标签内提取结果逐字节一致，并恰好提供 `dufs-0.49.7-x86_64-unknown-linux-gnu` 二进制（`6025624` 字节，SHA-256 `4dd74e3164fbffcb3765c2c33c518ab9c24e7571bd23f5206fc6ce3802ddd66b`）和 `dufs-0.49.7-x86_64-unknown-linux-gnu.sha256` 校验和文件（`103` 字节，文件自身 SHA-256 `a282bbad570d55eabef56d41a0501b6182acac2d802ff569c90f4e014cd120b4`）；后者精确描述前者。这两个 SHA-256 只固定公开资产的传输内容，不为 tag 或源码提交提供发布者签名。`0.49.7` 是面向协议严格性、共享路径一致性、取消安全、资源耗尽防护、崩溃恢复和发布供应链的补丁版本，不改变 CLI、YAML、SQLite schema 或既有成功响应契约。

### 安全强化

- HTTP 内容协商与浏览器 JSON 入口现在严格解析 `Accept` 和 `Content-Type`：缺少 `=` 的媒体参数、畸形参数及重复 `Content-Type` 不再被宽松接受；认证与 CSRF 失败也在读取受限响应正文前完成可信分类，避免正文超时或大小限制掩盖真实安全结果。
- 路径协调器把多组件符号链接形成的根内语义别名纳入同一互斥域；路径等待、上传预检、列表搜索和就绪探测同时使用有界请求准入，避免别名绕锁或慢文件系统工作耗尽连接与执行资源。
- 已经交给阻塞线程、状态 actor 或上传任务的工作在外层 HTTP future 取消后仍由 `TaskTracker` 跟踪并继续持有对应许可；上传超时、预检取消、路径解析取消和维护扫描取消不再提前释放稀缺槽位或冲突路径租约。
- 就绪探测增加单槽快速失败：昂贵的共享根、磁盘空间和 SQLite 检查尚未真正结束时，后续请求返回带 `Retry-After` 的 `503`，不会因调用方断开而制造无界后台探针。

### 问题修复

- 维护扫描删除过期上传状态前会重新核对活动登记与候选身份，不再依据过期快照误删已经重新活跃的状态文件。
- 现存 SQLite 主库携带合法 hot rollback journal 时允许 SQLite 完成崩溃恢复；主库、sidecar 类型、身份、schema 和共享根绑定仍按原有失败关闭规则验证。
- 路径等待器不再广播唤醒同一路径的全部排队者，避免形成 O(N²) 惊群；请求槽饱和时返回明确、可重试的 `429` 协议响应。
- 上传状态查询遇到临时 `503` 或未知服务端结果时，浏览器会保留原 upload ID 并允许重新查询；路径等待响应若夹带不允许的 length/offset 字段则作为协议错误拒绝。
- 状态 actor 会跳过回复端已经关闭的只读命令，避免无消费者查询继续占用数据库；可能改变持久状态的命令仍必须执行，不能因调用方取消而丢弃。
- 列表与搜索的首次及最终 metadata 探针现在都受同一并发准入保护；游标分页继续沿用轻量路径，不额外占用搜索槽。

### 性能与资源治理

- 批量上传预检和持久状态路径扫描均采用固定并发上限；扫描改为批量解析候选路径，减少阻塞任务数量和重复语义解析。
- 根内性验证改为一次锚定探测；深路径协调先解析完整父路径，并以有界搜索定位缺失祖先，避免逐层重复执行完整根路径解析。
- 维护扫描许可、上传槽、路径请求槽和预检槽都与实际底层工作的生命周期绑定，不再与可能提前结束的 HTTP 等待生命周期绑定。

### 发布与维护

- 正式发布 E2E 脚本已纳入本地与 CI 的 Bash/ShellCheck 清单，避免关键签名发布路径脱离 Shell 静态检查；当前规范同步记录实际受检的六个 Bash 源，不再沿用旧计数。
- 发布说明提取器会忽略 fenced code block 内形似版本标题的 Markdown；工作流静态检查器按语法结构和精确上下文验证安全门，不再因其他位置出现相同字符串而误绿。
- GitHub Actions 重跑发布时重新绑定当前 tag/SHA 的依赖审计 run，不再复用同一 workflow run 中更早 attempt 的过期审计成功结果。

### 测试

- 新增 Rust 单元与集成回归，覆盖畸形 HTTP 头、符号链接语义别名、SQLite hot journal、维护扫描竞态、路径等待惊群、各类取消时序、状态 actor 丢弃、安全准入和就绪探测过载响应。
- 新增前端单元与 Chromium/Firefox 场景，覆盖上传状态 `503` 重查、严格路径等待协议以及列表探针并发边界；发布脚本自测增加代码围栏与工作流 mutation fixtures。

### 升级说明

- 从 `0.49.6` 升级无需修改 CLI、YAML、SQLite schema 或外部 API；替换二进制并正常重启即可。过载路径可能更早返回带恢复提示的 `429` 或 `503`，客户端应遵循 `Retry-After` 或既有 recovery 字段，而不是立即无界重试。
- 从 `0.48.x` 或更早版本直接升级时，仍必须完成 `0.49.0` 的全部破坏性迁移，尤其是把 `--auth`/`-a` 迁入受保护的 YAML `auth`，并为反向代理显式配置窄范围 `trusted-proxies`。

### 文档

- 同步生产验签示例、功能取舍清单与新手指南中的当前 Cargo 版本，并记录本轮完整审查覆盖的协议、存储、并发、前端、部署和发布边界。
...
- GitHub Release 发布阶段不再假定按标签 REST 端点可以回读草稿；GraphQL 已经绑定目标 Release 后，后续元数据与资产枚举使用该 Release 的精确数字 ID，因此刚创建但按标签查询仍返回 `404` 的隐藏草稿可以继续完成校验、资产上传、逐字节复核和公开，仍不会放宽 tag、标题、说明、摘要或资产集合约束。

### 升级说明

- 从 `0.49.2` 升级时无需修改 CLI、YAML、SQLite schema 或外部 API，但仍应检查状态目录祖先权限与 1 GiB 原始 SQLite 主库限制；已经按源码部署 `0.49.3`、`0.49.4` 或 `0.49.5` 的环境只需替换二进制并正常重启。
- 从 `0.48.x` 或更早版本直接升级时，仍必须完成 `0.49.0` 的全部破坏性迁移，尤其是把 `--auth`/`-a` 迁入受保护的 YAML `auth`，并为反向代理显式配置窄范围 `trusted-proxies`。

### 发布与维护

- 附注标签 `v0.49.5` 精确指向提交 `f697ca997017e724bce1cf3de7a169495f0e599e`；同一 tag/SHA 的只读 CI、依赖审计、正式签名包 E2E 与只读构建均成功，写权限发布 job 随后创建了匹配的隐藏草稿，但按标签 REST 端点对该草稿返回 `404`，工作流在上传或公开前失败关闭。草稿资产为零，没有公开 GitHub Release、便捷二进制或校验和资产；标签保持不可变，累计变更与草稿回读修复由 `v0.49.6` 承接。

### 测试

- 发布工作流静态检查与 mutation fixtures 覆盖“GraphQL 可见草稿、按标签 REST 返回 `404`”的创建后回读路径，要求后续状态和分页资产查询绑定精确 Release ID，并拒绝退回按标签读取、放宽 ID 绑定或绕过既有元数据与资产校验。

### 文档

- 同步运维校验示例、功能取舍清单与入门指南的当前 Cargo 版本，并明确 `v0.49.3` 至 `v0.49.5` 均无公开 Release 或资产以及由 `0.49.6` 累计替代的关系。

## [0.49.5] - 2026-08-24

> **发布状态：** 附注标签 `v0.49.5` 精确指向提交 `f697ca997017e724bce1cf3de7a169495f0e599e`；同一 tag/SHA 的只读 CI、依赖审计、正式签名包 E2E 与只读构建均成功，写权限发布 job 随后创建了匹配的隐藏草稿，但按标签 REST 端点 `releases/tags/v0.49.5` 对该草稿返回 `404`，工作流在上传或公开前失败关闭。草稿资产为零，没有公开 GitHub Release、便捷二进制或校验和资产；标签保持不可变，其累计变更与草稿回读修复由 `0.49.6` 承接。

### 问题修复

- 正式签名包 E2E 不再自动发现或运行 GitHub `ubuntu-24.04` 宿主中偶然存在的未固定 Edge；隔离正式发布门只运行按锁定 Playwright 版本显式安装的 Chromium 和 Firefox，避免未固定宿主浏览器在含特殊字符的超长发布 stage 下因 `ProcessSingleton` 的 `SingletonSocket` 超出 Unix socket 路径上限而使门禁失败。普通本地检查在已安装 Edge 时仍会运行该可选覆盖，不缩减开发者可选的兼容性验证。

### 升级说明

- 从 `0.49.2` 升级时无需修改 CLI、YAML、SQLite schema 或外部 API，但仍应检查状态目录祖先权限与 1 GiB 原始 SQLite 主库限制；已经按源码部署 `0.49.3` 或 `0.49.4` 的环境只需替换二进制并正常重启。
- 从 `0.48.x` 或更早版本直接升级时，仍必须完成 `0.49.0` 的全部破坏性迁移，尤其是把 `--auth`/`-a` 迁入受保护的 YAML `auth`，并为反向代理显式配置窄范围 `trusted-proxies`。

### 发布与维护

- 附注标签 `v0.49.4` 精确指向提交 `c7f05889f670047c4a60cb9a767645547ce5252e`，并已包含 `v0.49.3` 的全部变更及非 root 归档权限修复；其只读 CI 与依赖审计成功，但正式签名包 E2E 在公开前因未固定宿主 Edge 的 `SingletonSocket` 路径过长而失败关闭。对应 GitHub Release API 返回 `404`，没有草稿或公开 Release，公开资产为零；标签保持不可变，累计变更与门禁修复由 `v0.49.5` 承接。

### 测试

- 发布工作流静态结构检查现在锁定“隔离正式发布门禁禁用宿主 Edge 自动发现、普通本地检查在已安装时保留 Edge 覆盖”的边界，并通过条件翻转 mutation 证明配置方向一旦倒置就会被拒绝。
- 正式签名包 E2E 仍在包含空格和 shell 元字符的超长输出路径中运行未缩短的完整发布入口，并保留 Chromium/Firefox 必需矩阵、归档权限、签名、SBOM、清单和原子发布验证。

### 文档

- 同步运维校验示例、功能取舍清单与入门指南的当前 Cargo 版本，并明确 `v0.49.3`、`v0.49.4` 均无公开 Release 或资产以及由 `0.49.5` 承接的关系。

## [0.49.4] - 2026-08-24

> **发布状态：** 附注标签 `v0.49.4` 精确指向提交 `c7f05889f670047c4a60cb9a767645547ce5252e`，并已包含 `v0.49.3` 的全部变更及非 root 归档权限修复；其只读 CI 与依赖审计成功，但正式签名包 E2E 因 GitHub `ubuntu-24.04` 宿主中偶然存在的未固定 Edge 在含特殊字符的超长发布 stage 下触发 `SingletonSocket` 路径过长而失败关闭。对应 GitHub Release API 返回 `404`，没有草稿或公开 Release，公开资产为零；标签保持不可变，其累计变更已进入不可变的 `v0.49.5` 标签，并与后续正式发布浏览器隔离及隐藏草稿回读修复一并由 `0.49.6` 承接。

### 问题修复

- 正式发布归档自测解包现在显式保留归档中已经规范化的 `0755/0644` 公开权限；非 root 发布 runner 即使继承 `umask 077`，也不会把目录、文档和脚本误解包为 `0700/0600` 后触发确定性的权限校验失败。

### 升级说明

- 从 `0.49.2` 升级时无需修改 CLI、YAML、SQLite schema 或外部 API，但仍应检查状态目录祖先权限与 1 GiB 原始 SQLite 主库限制；已经按源码部署 `0.49.3` 的环境只需替换二进制并正常重启。
- 从 `0.48.x` 或更早版本直接升级时，仍必须完成 `0.49.0` 的全部破坏性迁移，尤其是把 `--auth`/`-a` 迁入受保护的 YAML `auth`，并为反向代理显式配置窄范围 `trusted-proxies`。

### 发布与维护

- `v0.49.3` 的非 root 归档权限自测缺陷已在 `v0.49.4` 修复；后者又因独立的未固定宿主 Edge 路径边界在公开前失败关闭，两个标签均保持不可变且没有公开资产，累计变更由 `v0.49.5` 承接。

### 测试

- 只读 CI 的发布打包自测现在显式运行在 `umask 077` 下，使普通非 root runner 在常规门禁中持续覆盖与正式打包一致的私有权限环境。

## [0.49.3] - 2026-08-24

> **发布状态：** 附注标签 `v0.49.3` 精确指向提交 `c0123c412c8c165c37540dc510df695c150da4e6`，但正式签名包门禁发现非 root 归档权限自测缺陷后失败关闭；没有公开 GitHub Release、便捷二进制或校验和资产。其变更已依次进入不可变的 `v0.49.4` 与 `v0.49.5` 标签，并与后续归档权限、正式发布浏览器隔离及隐藏草稿回读修复一并由 `0.49.6` 承接。
...
- 正式发布 stage 的目录 fd 锚点固定引用打包主进程；即使 npm/Node 启动的孙进程关闭或复用继承 fd，隔离的 Cargo home、构建目录及其他 stage 子路径仍绑定已加锁的输出目录 inode，不会退化为可换名的实体路径。
...
- Unix socket 覆盖测试通过持续打开的共享根目录 fd 使用短绑定路径，在超长隔离临时目录下仍创建并核验同一个根内 socket，不会在进入服务逻辑前触发 `sun_path` 长度限制。
...
[未发布]: https://github.com/isarmg/dufs-ram/compare/v0.49.7...main
[0.49.7]: https://github.com/isarmg/dufs-ram/compare/v0.49.6...v0.49.7
[0.49.6]: https://github.com/isarmg/dufs-ram/compare/v0.49.5...v0.49.6
[0.49.5]: https://github.com/isarmg/dufs-ram/compare/v0.49.4...v0.49.5
[0.49.4]: https://github.com/isarmg/dufs-ram/compare/v0.49.3...v0.49.4
