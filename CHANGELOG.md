...
> **发布说明：** `0.49.5` 是未形成公开 GitHub Release 的 `0.49.3` 和 `0.49.4` 的替代补丁版本，包含两者的全部安全强化、问题修复、发布流程与文档变更，以及本版本的正式发布浏览器隔离修复；完整累计清单见 [0.49.4 变更记录](https://github.com/isarmg/dufs-ram/blob/v0.49.5/CHANGELOG.md#0494---2026-08-24) 与 [0.49.3 变更记录](https://github.com/isarmg/dufs-ram/blob/v0.49.5/CHANGELOG.md#0493---2026-08-24)。
...
> **发布状态：** 附注标签 `v0.49.4` 精确指向提交 `c7f05889f670047c4a60cb9a767645547ce5252e`，并已包含 `v0.49.3` 的全部变更及非 root 归档权限修复；其只读 CI 与依赖审计成功，但正式签名包 E2E 因 GitHub `ubuntu-24.04` 宿主中偶然存在的未固定 Edge 在含特殊字符的超长发布 stage 下触发 `SingletonSocket` 路径过长而失败关闭。对应 GitHub Release API 返回 `404`，没有草稿或公开 Release，公开资产为零；标签保持不可变，其累计变更与门禁修复由 `0.49.5` 承接。
...
> **发布状态：** 附注标签 `v0.49.3` 精确指向提交 `c0123c412c8c165c37540dc510df695c150da4e6`，但正式签名包门禁发现非 root 归档权限自测缺陷后失败关闭；没有公开 GitHub Release、便捷二进制或校验和资产。其变更已进入不可变的 `v0.49.4` 标签，并与后续归档权限及正式发布浏览器隔离修复一并由 `0.49.5` 承接。
...
[未发布]: https://github.com/isarmg/dufs-ram/compare/v0.49.5...main
